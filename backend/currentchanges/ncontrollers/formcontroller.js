...
const createForm = async (req, res) => {
  ...
  try {
    ...
    const user_id = req.user?.id; // comes from token

    if (!user_id)
      return res.status(401).json({ success: false, message: "Unauthorized" });

    if (!title || !title.trim()) {
      return res.status(400).json({ success: false, error: "Form title required" });
    }
...
// POST submit form response submitFormResponse
...
// GET all responses for a form getAllResponses
...
// GET single response details getSingleResponse
